...
create materialized view labels as
  ...
  with all_labels as (
    select p.uniquepid, p.patienthealthsystemstayid, apr.patientunitstayid, p.unitvisitnumber, apr.actualhospitalmortality, apr.actualiculos
      from patient as p
      inner join apachepatientresult as apr
        on p.patientunitstayid = apr.patientunitstayid
      -- exclude anyone who doesn't have at least 24 hours of data
      where apr.actualiculos >= 1
    )
  select al.patientunitstayid, al.actualhospitalmortality, al.actualiculos
